...
# ----- Config: address of your publisher client agent -----
...
# ----- State -----
SEND_QUEUE: "Queue[Dict[str, Any]]" = Queue()   # HTTP -> Agent
RESULTS: Dict[str, Dict[str, Any]] = {}         # request_id -> {"status": "pending"|"done", ...}
...
def push_handler():
    ...
    SEND_QUEUE.put({"request_id": rid, "visibility": visibility, "payload": payload})
...
# ----- Bridge agent (drains queue, sends to publisher client, receives outcome) -----
bridge = Agent(
    name="ui-gateway",
    ...
    try:
        while True:
            ...
            msg = PushDoc(
                ...
                request_id=rid,
                callback_address=bridge.address,  # receive PublishOutcome here
            )
...
def run_flask():
    # Use threaded=True so requests don’t block each other
