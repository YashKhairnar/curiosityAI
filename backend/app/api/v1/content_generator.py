...
def content_generator():
    ...
    Body JSON:
    {
      ...
      "preferred_stack": "string (optional)",
      "num_ideas": 3,                # optional, default 3, max 5
      "num_research_titles": 5       # optional, default 5, max 10
    }
    """
    data = request.get_json(silent=True) or {}
    summary = (data.get("summary") or "").strip()
    preferred_stack = (data.get("preferred_stack") or "").strip() or None
    num_ideas = int(data.get("num_ideas") or 3)
    ...
    if coding_related:
        ideas = generate_multi_code_and_docs(summary, preferred_stack, num_ideas)
        result.update({
